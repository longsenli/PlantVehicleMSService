...
public interface CarDriveRecordMapper {
    ...
    @Select( "select carID,driverName, carType, department, driverPhone, comeTime,comeRecorder,goTime,goRecorder from (\n" +
            ...
            " FROM tb_cardriverecord where carID= #{id} and status = '1'  and comeTime >= #{startTime}  and comeTime <= #{endTime}\n" +
            ") a left join tb_carinfo b on a.carID = b.carLicence order by comeTime desc  ")
    List<Map<Object, Object>>  selectRecordByFilter(String id,String startTime,String endTime );
